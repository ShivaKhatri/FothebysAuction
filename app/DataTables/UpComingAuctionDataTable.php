...
class UpComingAuctionDataTable extends DataTable
{
    /**
     ...
     */
    public function dataTable($query)// to give details of the upcoming auctions
    {
...
//                dd($wow);
    ...
    {
        return $model->newQuery()->select('id', 'description', 'location','session','date')->where('date','>',date('Y-m-d'))->where('status','=',1);
    ...
    {
        return 'UpComingAuction_' . date('YmdHis');
    }
}
